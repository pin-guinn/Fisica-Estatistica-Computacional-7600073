...
PROGRAM tarefa5
  ...
  OPEN(UNIT=10, FILE="saida-5-tempo.dat", STATUS="UNKNOWN")

  N = (/ 50, 100, 200, 400, 800, 1600, 3600, 7200/) !! Indo um pouco mais além...
  
  DO j=1, 8
    CALL GerarSerie(N(j), 0.04d0, 2d0, 4d0, 4d0*PI, 2.5d0*PI)

    CALL CPU_TIME(t1)
    CALL Fourier(N(j), 0.04d0)
    CALL CPU_TIME(t2)
